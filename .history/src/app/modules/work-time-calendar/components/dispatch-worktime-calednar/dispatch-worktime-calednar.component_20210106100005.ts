...
export class DispatchWorktimeCalednarComponent implements OnInit {
  ...
  ngOnInit() {
    this.worktimeservice.getSubActivityCalender( this.calender_current_week , this.selected_Activity).pipe(takeUntil(this.unsubscribe)).subscribe(
        data => {
  ...
  changeCalenderWeekIndex(items) {
    this.calender_current_week = items;
    this.worktimeservice.selected_Activity = items;
    console.log(items);
    // this.ngOnInit();
}
