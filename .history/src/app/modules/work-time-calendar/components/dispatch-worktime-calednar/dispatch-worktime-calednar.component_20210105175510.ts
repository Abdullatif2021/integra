...
export class DispatchWorktimeCalednarComponent implements OnInit {
  ...
  ngOnInit() {
    this.worktimeservice.getSubActivityCalender( this.calender_current_week , this.ggggggg).pipe(takeUntil(this.unsubscribe)).subscribe(
        data => {
...
changeActivity(event) {
  ...
  this.ggggggg = this.selected_Activity.map(p => p.id);
  console.log(event)
  this.ngOnInit();
}
